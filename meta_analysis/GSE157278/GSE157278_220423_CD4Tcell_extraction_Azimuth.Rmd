...
select.CD4T.meta.data <- c("orig.ident", "Cell", "sample", "disease", "disease_duration",
                           "condition", "age", "sex", "immunosuppressant", "immunosuppressant_duration",
                           "batch")
genes <- c("CD3E", "CD4", "CD8A", "FOXP3", "IL7R",
           "CD74")
...
VlnPlot(query,
        ...
        ncol = 3,
        pt.size = 0)
ggsave(paste0(prefix_output, "_check_QC_vlnPlot_CD4T_extraction.pdf"),
       width = 5,
       height = 10)
```
...
DimPlot(object = query,
        ...
        label = TRUE) + NoLegend()
ggsave(paste0(prefix_output, "_check_dimplot_CD4T_extraction.pdf"),
       width = 5,
       height = 10)
...
ggsave(paste0(prefix_output, "_check_dotplot_CD4T_extraction.pdf"),
       width = 5,
       height = 10)
```
...
ggsave(paste0(prefix_output, "_check_featureplot_CD4T_extraction.pdf"),
       width = 10,
       height = 20)
```

# 16. Extraction of CD4+Tcell
...
pbmc_CD4T.meta.data <- pbmc_CD4T.meta.data %>%
  select(all_of(select.CD4T.meta.data))
...
saveRDS(pbmc_CD4T.meta.data,
        file = paste0(prefix_output, "_CD4T_MetaData.rds"))

saveRDS(pbmc_CD4T,
        file = paste0(prefix_output, "_CD4T_AssayData.rds"))
```

# 18. Save workspace

```{r save.image}
save.image(file = paste0(prefix_output, "_220423_CD4Tcell_extraction_Azimuth.RData"))
```
