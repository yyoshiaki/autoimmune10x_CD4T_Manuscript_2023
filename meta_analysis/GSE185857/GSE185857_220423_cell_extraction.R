...
for (s in df.metadata$sample){
  mat <- readMM(paste0("./data/",project.name,"/", s ,".matrix.mtx.gz"))
  barcodes <- read_tsv(paste0("./data/",project.name,"/", s ,".barcodes.tsv.gz"),
                       col_names = FALSE)$X1
  features <- read_tsv(file = paste0("./data/",project.name,"/", s ,".features.tsv.gz"),
                       col_names = FALSE)$X2
