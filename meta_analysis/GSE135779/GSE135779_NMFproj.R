...
df <- df %>% separate(condition, c('group', 'SLEDAI'), sep = ';')
...
#                         labels = c("Healthy", "Asymptomatic", "Mild", "Moderate", "Severe", "Critical"))
df.plot <- df %>% group_by(clusterL2, group) %>%
  summarise(NMF_0=mean(NMF_0), NMF_1=mean(NMF_1), NMF_2=mean(NMF_2),
            NMF_3=mean(NMF_3), NMF_4=mean(NMF_4), NMF_5=mean(NMF_5),
            NMF_6=mean(NMF_6), NMF_7=mean(NMF_7), NMF_8=mean(NMF_8),
            NMF_9=mean(NMF_9), NMF_10=mean(NMF_10), NMF_11=mean(NMF_11)) %>%
  mutate(description = paste0(clusterL2,'_',group))
...
pdf(paste0(prefix, '_CD4T_NMF_clusterL2_group_heatmap.pdf'), width = 6, height = 24)
p
dev.off()
