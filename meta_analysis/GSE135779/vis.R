...
project.name <- "GSE135779"


df.metadata <- read_csv(paste0('data/', project.name,'/', project.name,'_metadata.csv'))
df.metadata <- df.metadata %>% separate(condition, c('group', 'SLEDAI'), sep = ';')
...
p <- ggplot(df.output, aes(x=clusterL2, y=freq, color=group), scale = "width") + 
  geom_quasirandom(dodge.width=1) +
  theme(axis.text.x = element_text(angle = 90))
p  
...
# change disease name depending on the dataset
df.ttest <- df.output  %>% 
  group_by(clusterL2, group) %>% 
  summarise(freq = list(freq)) %>% 
  spread(group, freq) %>% 
  group_by(clusterL2) %>% 
  mutate(p_value = t.test(unlist(cSLE), unlist(cHD))$p.value,
         t_value = t.test(unlist(cSLE), unlist(cHD))$statistic)

df.ttest <- df.ttest %>% select(c(clusterL2, p_value, t_value))
df.ttest['p.adj'] <- p.adjust(df.ttest$p_value)
write_csv(df.ttest, paste0('output/', project.name,'/', project.name, '_queryL2_ttest.child.csv'))
...
df.ttest <- df.output  %>% 
  ...
  spread(group, freq) %>% 
  group_by(clusterL2) %>% 
  mutate(p_value = t.test(unlist(aSLE), unlist(aHD))$p.value,
         t_value = t.test(unlist(aSLE), unlist(aHD))$statistic)

df.ttest <- df.ttest %>% select(c(clusterL2, p_value, t_value))
df.ttest['p.adj'] <- p.adjust(df.ttest$p_value)
write_csv(df.ttest, paste0('output/', project.name,'/', project.name, '_queryL2_ttest.adult.csv'))
