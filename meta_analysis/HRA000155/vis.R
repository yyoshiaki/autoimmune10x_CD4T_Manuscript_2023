...
p <- ggplot(df.output, aes(x=clusterL2, y=freq, color=condition), scale = "width") + 
  geom_quasirandom(dodge.width=1) +
  theme(axis.text.x = element_text(angle = 90))
p  

ggsave(paste0('output/', project.name,'/', project.name, '_queryL2_ggbeeswarm_cond.pdf'), width = 8, height = 4)
...
dis <- "ACPA-"
df.ttest <- df.output  %>% 
  mutate(condition = replace_na(condition, "HC")) %>%
  group_by(clusterL2, condition) %>% 
  summarise(freq = list(freq)) %>% 
  filter(clusterL2 != "TcmPHLDA3") %>%
...
df.ttest <- df.ttest %>% select(c(clusterL2, p_value, t_value))
df.ttest['p.adj'] <- p.adjust(df.ttest$p_value)
write_csv(df.ttest, paste0('output/', project.name,'/', project.name, '_queryL2_ttest_ACPAneg.csv'))
...
df.ttest <- df.ttest %>% select(c(clusterL2, p_value, t_value))
df.ttest['p.adj'] <- p.adjust(df.ttest$p_value)
write_csv(df.ttest, paste0('output/', project.name,'/', project.name, '_queryL2_ttest_ACPApos.csv'))
