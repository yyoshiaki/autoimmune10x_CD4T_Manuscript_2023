...
ggsave(paste0('output/', project.name,'/', project.name, '_queryL2_ggbeeswarm.Status.pdf'), width = 8, height = 4)


# set value 0 in the cluster in the samples was not detected
df.output <- df.output %>% 
  pivot_wider(values_from = freq, names_from = clusterL2,
              id_cols = c(sample, condition), values_fill = 0) %>%
  pivot_longer(!c(sample,condition), names_to = "clusterL2", values_to = "freq")
...
df.ttest <- df.output  %>% 
  ...
  group_by(clusterL2) %>% 
  mutate(p_value = t.test(unlist(.data[[dis]]), unlist(Healthy))$p.value,
         t_value = t.test(unlist(.data[[dis]]), unlist(Healthy))$statistic)
