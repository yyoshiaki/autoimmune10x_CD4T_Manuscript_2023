...
df.metadata = read.csv("./data/GSE163314/GSE163314_metadata.csv", header = TRUE)
df.metadata

holder <- list()

for (i in df.metadata$sample) {
  query <- Read10X(data.dir = paste0('./data/GSE163314/', i))
  query <- CreateSeuratObject(counts = quer, project = i)
  holder <- c(holder, query)
}

holder

query <- merge(holder[[1]], y=c(holder[[2]],holder[[3]],holder[[4]],holder[[5]],holder[[6]],holder[[7]],holder[[8]]), add.cell.ids = df.metadata$batch, project = "GSE163314")
head(colnames(query.data))
tail(colnames(query.data))
